<div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-4 col-md-6 col-sm-9 ">
                    <a href="{{route('participation.panel')}}">
                        <div class="services-item text-center mt-10 wow fadeIn hoverable" style="border-radius: 10px;" data-wow-duration="1500ms" data-wow-delay="0ms">
                            <h3 class="title">Panels citoyens</h3>
                            <img class="mt-30 mb-30" src="assets/images/icon-panel.svg" width="20%" height="20%"  alt="">
                            <p>Avoids pleasure itself, because it is pleasure but because</p>
                            <a href="{{route('participation.panel')}}">Voir plus <i class="fal fa-long-arrow-right"></i></a>
                        </div>
                    </a>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-9">
                    <a href="{{route('participation.comite')}}">
                        <div class="services-item text-center mt-10 wow fadeIn hoverable" style="border-radius: 10px;" data-wow-duration="1500ms" data-wow-delay="400ms">
                            <h3 class="title" style="font-size: larger;">Comites consultatifs</h3>
                            <img class="mt-30 mb-40" src="assets/images/icon-comite.svg" width="20%" height="20%"  alt="">
                            <p>Avoids pleasure itself, because it is pleasure but because</p>
                            <a href="{{route('participation.comite')}}">Voir plus <i class="fal fa-long-arrow-right"></i></a>
                        </div>
                    </a>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-9">
                    <a href="{{route('participation.sondage')}}">
                        <div class="services-item text-center mt-10 wow fadeIn hoverable" style="border-radius: 10px;" data-wow-duration="1500ms" data-wow-delay="800ms">
                            <h3 class="title">Sondages</h3>
                            <img class="mt-30 mb-30" src="assets/images/icon-sondage.svg" width="20%" height="20%"  alt="">
                            <p>Avoids pleasure itself, because it is pleasure but because</p>
                            <a href="{{route('participation.sondage')}}">Voir plus <i class="fal fa-long-arrow-right"></i></a>
                        </div>
                    </a>
                </div>
            </div>
        </div>

